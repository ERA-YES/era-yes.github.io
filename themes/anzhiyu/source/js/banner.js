const body = document.getElementById("hexo-bilibili-banner-app");

const lerp = (start, end, amt) => (1 - amt) * start + amt * end; // 计算线性插值

const barnerData = await fetch('banner-assets/data.json')
let allImagesData = await barnerData.json()
...
function initItems() {
  ...
  if (layers.length <= 0) {
    ...
    for (let i = 0; i < cloneData.length; i++) {
      ...
      layer.classList.add("hexo-bilibili-banner-layer");
      if (compensate !== 1) {
      ...
      child.classList.add("hexo-bilibili-banner-layer-item");
      child.src = item.src;
    ...
    body.style.display = "";
    layers = document.querySelectorAll(".hexo-bilibili-banner-layer");
  } else {
    // 窗口大小变动时重新计算内容
    const cloneData = JSON.parse(JSON.stringify(allImagesData))
